...
export default function FloatingContact() {
  ...
  const [whatsAppIntent, setWhatsAppIntent] = useState('HIRE');

  useEffect(() => {
    const handleOpen = (e) => {
      if (e?.detail?.intent) {
        setWhatsAppIntent(e.detail.intent);
      }
  ...
  return (
    <>
      ...
      <div className="fixed inset-x-0 bottom-0 z-[70] grid grid-cols-2 gap-2 border-t border-slate-200 bg-white p-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] shadow-[0_-6px_20px_rgba(15,23,42,0.14)] md:hidden">
        ...
        <button 
          type="button"
          onClick={() => { setWhatsAppIntent('HIRE'); setIsWhatsAppModalOpen(true); }}
          className="flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#0B6B2E] hover:bg-[#095725] px-3 py-2.5 text-sm font-extrabold text-white cursor-pointer transition-colors"
        >
      ...
      <div className="hidden md:flex fixed bottom-6 right-6 z-[60] flex-col gap-4">
        {/* WhatsApp Button */}
        <button
          type="button"
          onClick={() => { setWhatsAppIntent('HIRE'); setIsWhatsAppModalOpen(true); }}
          className="w-14 h-14 bg-green-500 text-white rounded-full flex items-center justify-center shadow-lg hover:bg-green-600 hover:-translate-y-1 hover:shadow-green-500/30 transition-all duration-200 cursor-pointer"
