  ...
  return (
    <>
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        {/* Direct Worker Contact Banner — Right After Breadcrumbs / Header */}
        <DirectContactBanner
          serviceName={job.role?.name || 'Workers'}
          cityName={job.location?.name || 'your city'}
          serviceSlug={job.role?.slug}
        ...
        <div className="space-y-12">
          {job.role && <RelatedRoles currentRoleSlug={job.role.slug} />}
          {job.location && <RelatedLocations currentLocationSlug={job.location.slug} />}
          {job.role && <RelatedServices currentServiceSlug={job.role.slug} />}
        </div>
      </div>
    </>
  );
